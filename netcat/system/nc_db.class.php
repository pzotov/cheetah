        ...
        // Query was a select
            ...
            if (!is_bool($this->result)) {
                ...
                $this->result_output_type = $output;
                    ...
                    if ($output == ARRAY_N) { $fetch_function = 'mysql_fetch_row'; }
                elseif ($output == ARRAY_A) { $fetch_function = 'mysql_fetch_assoc'; }
                                       else { $fetch_function = 'mysql_fetch_object'; }
...
//        $this->func_call = "\$db->get_row(\"$query\",$output,$y)";
    ...
    public function get_col($query = null, $x = 0, $index_x = null) {
        ...
        for ($i = 0, $last = count($this->last_result); $i < $last; $i++) {
            if ($index_x !== null) {
...
//        $this->func_call = "\$db->get_results(\"$query\", $output)";
        ...
        elseif (is_array($this->last_result)) {
            $result = array();
